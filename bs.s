!----------------- bs.s file ------------------------------------
	BOOTSEG = 0x9000
	SSP	= 32*1024	! Start Segment Ponter?
	...
	!--------------------------------------------------------
	mov ax, #BOOTSEG	! set extra segment pointer to 0x9000
	mov es, ax
	xor bx, bx	! clear BS = 0
	!------------------------------------------
...
msg:	.asciz  "\n\rError!\n\r"
	...
	! setds(segment) : set DS to segment to R/W memeory outside BOOTSEG
	!_setds:
		push bp
		mov  bp, sp
		mov  ax, 4[bp]
