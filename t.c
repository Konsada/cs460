...
{
//	myprintf("scheduler()\n");
//	myprintf("running_pid:%d -> status: %d\n", running->pid, running->status); 
//	getc();
...
PROC *kfork() // create a child process, begin from body()
{
	...
	child->ksp = &(child->kstack[SSIZE-11]);   // proc saved sp
// work?
	child->kstack[SSIZE-10] = 0x1000;
	child->kstack[SSIZE-11] = 0x1000;
...
int main()
{
	...
	init();
//myprintf("finished init()\n");
//getc();
	myprintf("P%d running\n", running->pid);
	kfork();
//myprintf("finished kfork()\n");
//getc();
	while(1){
		myprintf("P0 running\n");
		if(nproc==2 && proc[1].status != READY)
			myprintf("no runable process, system halts\n");
		while(!readyQueue && !freeList);
